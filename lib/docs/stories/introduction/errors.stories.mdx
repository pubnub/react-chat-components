...
`onError` property of the `Chat` provider component and passing it to a custom callback function. This
function will be called each time a JavaScript exception is raised within the components code with the
...
To enable the retry behavior you must configure the `retryOptions` property on the `Chat`
provider:
...
Custom hooks have their own way of handling errors. Every custom hook exposes the most recent error it
ran into as the last element of its output array. 
