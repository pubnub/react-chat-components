...
are internally used to style different parts of each component. This document provides the full list
of available variables along with their default values.

For example, to remove the default padding around `MessageInput`, you can simply redefine the
varaible in your CSS file:
...
In cases where the variable value is redefined by a particular theme in use, a higher CSS
specification needs to be applied. For example, to change the background of `MessageList` in the
dark mode, you must use a component class name instead of the `:root` element:
