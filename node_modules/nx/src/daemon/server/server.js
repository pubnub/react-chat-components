...
const handle_output_contents_1 = require("./handle-output-contents");
...
function handleMessage(socket, data) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        if (watcherError) {
            yield (0, shutdown_utils_1.respondWithErrorAndExit)(socket, `File watcher error in the workspace '${workspace_root_1.workspaceRoot}'.`, watcherError);
        }
        if (lockFileChanged()) {
        ...
        if (payload.type === 'REQUEST_PROJECT_GRAPH') {
            ...
            yield handleResult(socket, yield (0, handle_output_contents_1.handleRecordOutputsHash)(payload));
        }
        else if (payload.type === 'OUTPUTS_HASHES_MATCH') {
            yield handleResult(socket, yield (0, handle_output_contents_1.handleOutputsHashesMatch)(payload));
        }
        else {
            yield (0, shutdown_utils_1.respondWithErrorAndExit)(socket, `Invalid payload from the client`, new Error(`Unsupported payload sent to daemon server: ${unparsedPayload}`));
        }
