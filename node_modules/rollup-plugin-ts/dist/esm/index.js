...
import { getAppropriateEcmaVersionForBrowserslist, normalizeBrowserslist, browsersWithSupportForEcmaVersion } from 'browserslist-generator';
import { randomBytes, createHmac } from 'crypto';
import TSModule from 'typescript';
import browserslistModule from 'browserslist';
import { createFilter } from '@rollup/pluginutils';
import { camelCase, matchAll } from '@wessberg/stringutil';
import { ensureNodeFactory } from 'compatfactory';
import { preserveNode, cloneNode, setParentNodes } from 'ts-clone-node';
...
const AMBIENT_EXTENSIONS = new Set([
    D_TS_EXTENSION,
    D_TS_MAP_EXTENSION,
    D_MTS_EXTENSION,
    D_MTS_MAP_EXTENSION,
...
function inspect(item, depth = 4) {
    console.log(inspect$1(item, { colors: true, depth, maxArrayLength: 1000 }));
}
...
function selectDeclarationExtensionBasedOnFilename(filename, supportedExtensions, sourcemap = false) {
 ...
 * Gets the destination directory to use for declarations based on the given CompilerOptions and Rollup output options
 */
function getDeclarationOutExtension(outputOptions, supportedExtensions, chunk, sourcemap = false) {
 ...
 * Removes an export modifier from the given ModifiersArray
 */
function ensureHasDeclareModifier(modifiers, factory, typescript) {
    if (modifiers == null)
        return [factory.createModifier(typescript.SyntaxKind.DeclareKeyword)];
...
function getExportedSymbolFromExportSpecifier(specifier, parentIsTypeOnly, moduleSpecifier) {
    var _a;
    return {
        moduleSpecifier,
...
function createExportSpecifierFromNameAndModifiers({ name, modifiers, typescript, factory, isTypeOnly = false }) {
    if (hasDefaultExportModifier(modifiers, typescript)) {
        const propertyNameText = name;
        const nameText = "default";
        const exportSpecifier = factory.createExportSpecifier(isTypeOnly, propertyNameText === nameText ? undefined : factory.createIdentifier(propertyNameText), factory.createIdentifier(nameText));
        return {
            exportSpecifier,
...
function visitModuleDeclaration$9({ node, ...options }) {
    if (node.body == null)
        return;
    return options.childContinuation(node.body);
}

function visitNode$d({ node, ...options }) {
    if (options.typescript.isImportDeclaration(node)) {
        return visitImportDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isImportTypeNode(node)) {
        return visitImportTypeNode$2({ ...options, node });
    }
    else if (options.typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$9({ ...options, node });
    }
...
function visitClassDeclaration$5({ node, typescript, markAsExported, ...options }) {
...
function visitClassExpression$5({ node, typescript, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
    if (!hasExportModifier(node, typescript) || node.name == null)
        return;
    const { exportedSymbol } = createExportSpecifierFromNameAndModifiers({
...
function visitFunctionDeclaration$5({ node, typescript, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
    if (!hasExportModifier(node, typescript) || node.name == null)
        return;
    const { exportedSymbol } = createExportSpecifierFromNameAndModifiers({
...
function visitFunctionExpression$5({ node, typescript, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
    if (!hasExportModifier(node, typescript) || node.name == null)
        return;
    const { exportedSymbol } = createExportSpecifierFromNameAndModifiers({
...
function visitEnumDeclaration$5({ node, typescript, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
 ...
 */
function traceIdentifiersForImportEqualsDeclaration({ node, addIdentifier }) {
    addIdentifier(node.name.text);
}
 ...
 */
function traceIdentifiersForNode({ node, ...options }) {
    if (options.typescript.isBindingElement(node))
        traceIdentifiersForBindingElement({ ...options, node });
    else if (options.typescript.isClassDeclaration(node))
        traceIdentifiersForClassDeclaration({ ...options, node });
    else if (options.typescript.isEnumDeclaration(node))
        traceIdentifiersForEnumDeclaration({ ...options, node });
    else if (options.typescript.isFunctionDeclaration(node))
        traceIdentifiersForFunctionDeclaration({ ...options, node });
    else if (options.typescript.isImportClause(node))
        traceIdentifiersForImportClause({ ...options, node });
    else if (options.typescript.isNamespaceImport(node))
        traceIdentifiersForNamespaceImport({ ...options, node });
    else if (options.typescript.isImportSpecifier(node))
        traceIdentifiersForImportSpecifier({ ...options, node });
    else if (options.typescript.isExportSpecifier(node))
        traceIdentifiersForExportSpecifier({ ...options, node });
    else if (options.typescript.isImportEqualsDeclaration(node))
        traceIdentifiersForImportEqualsDeclaration({ ...options, node });
    else if (options.typescript.isIdentifier(node))
        traceIdentifiersForIdentifier({ ...options, node });
    else if (options.typescript.isInterfaceDeclaration(node))
        traceIdentifiersForInterfaceDeclaration({ ...options, node });
    else if (options.typescript.isTypeAliasDeclaration(node))
        traceIdentifiersForTypeAliasDeclaration({ ...options, node });
    else if (options.typescript.isVariableDeclaration(node))
        traceIdentifiersForVariableDeclaration({ ...options, node });
    else if (options.typescript.isExportAssignment(node))
        traceIdentifiersForExportAssignment({ ...options, node });
    else if (options.typescript.isCallExpression(node))
        traceIdentifiersForCallExpression({ ...options, node });
    else if (options.typescript.isNewExpression(node))
        traceIdentifiersForNewExpression({ ...options, node });
    else
        options.childContinuation(node);
}

function traceIdentifiers({ node, ...rest }, identifiers = new Set()) {
    return getIdentifiers({ ...rest, node }, identifiers);
}
function getIdentifiers({ node, ...rest }, identifiers) {
    traceIdentifiersForNode({
        ...rest,
        node,
        continuation: nextNode => getIdentifiers({ ...rest, node: nextNode }, identifiers),
        childContinuation: nextNode => rest.typescript.forEachChild(nextNode, nextNextNode => {
...
function visitVariableStatement$5({ node, typescript, sourceFile, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
    if (!hasExportModifier(node, typescript))
        return;
...
function visitInterfaceDeclaration$5({ node, typescript, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
    if (!hasExportModifier(node, typescript))
        return;
    const { exportedSymbol } = createExportSpecifierFromNameAndModifiers({
...
function visitModuleDeclaration$8({ node, typescript, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
    if (!hasExportModifier(node, typescript)) {
        if (node.body != null)
            return options.childContinuation(node.body);
...
function visitTypeAliasDeclaration$5({ node, typescript, markAsExported, ...options }) {
    // If the node has no export modifier, leave it as it is
    if (!hasExportModifier(node, typescript))
        return;
    const { exportedSymbol } = createExportSpecifierFromNameAndModifiers({
...
function visitExportDeclaration$6({ node, typescript, markAsExported }) {
    ...
    for (const exportSpecifier of node.exportClause.elements) {
        markAsExported(getExportedSymbolFromExportSpecifier(exportSpecifier, node.isTypeOnly, (_b = node.moduleSpecifier) === null || _b === void 0 ? void 0 : _b.text));
    }
}
...
function visitNode$c({ node, ...options }) {
    if (options.typescript.isClassDeclaration(node)) {
        return visitClassDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isClassExpression(node)) {
        return visitClassExpression$5({ ...options, node });
    }
    else if (options.typescript.isFunctionDeclaration(node)) {
        return visitFunctionDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isFunctionExpression(node)) {
        return visitFunctionExpression$5({ ...options, node });
    }
    else if (options.typescript.isEnumDeclaration(node)) {
        return visitEnumDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isInterfaceDeclaration(node)) {
        return visitInterfaceDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isTypeAliasDeclaration(node)) {
        return visitTypeAliasDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$8({ ...options, node });
    }
    else if (options.typescript.isVariableStatement(node)) {
        return visitVariableStatement$5({ ...options, node });
    }
    else if (options.typescript.isExportDeclaration(node)) {
        return visitExportDeclaration$6({ ...options, node });
    }
    else if (options.typescript.isExportAssignment(node)) {
        return visitExportAssignment$3({ ...options, node });
    }
}
...
function sourceFileBundler(options, ...transformers) {
    return context => bundle => {
        ...
        const factory = ensureNodeFactory((_a = context.factory) !== null && _a !== void 0 ? _a : typescript);
        ...
        if (firstEntrySourceFile != null) {
            ...
            const visitorOptions = {
                ...
                includedSourceFiles: new Set([firstEntrySourceFile.fileName]),
                declarationToDeconflictedBindingMap: new Map(),
        ...
        for (const updatedSourceFile of updatedSourceFiles) {
            ...
            for (const typeReferenceModule of (_b = options.sourceFileToTypeReferencesSet.get(updatedSourceFile.fileName)) !== null && _b !== void 0 ? _b : new Set()) {
...
function preserveSymbols(node, otherNode, options) {
    if (node === otherNode)
        return node;
    node._symbol = getSymbolAtLocation({ ...options, node: otherNode });
    return node;
}
...
function visitClassDeclaration$4(options) {
    ...
    if (!hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateClassDeclaration(node, node.decorators, removeDeclareModifier(node.modifiers, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitClassExpression$4(options) {
    ...
    if (!hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateClassExpression(node, node.decorators, removeDeclareModifier(node.modifiers, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitFunctionDeclaration$4(options) {
    ...
    if (!hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateFunctionDeclaration(node, node.decorators, removeDeclareModifier(node.modifiers, typescript), node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body), node, options);
}
...
function visitFunctionExpression$4(options) {
    ...
    if (!hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateFunctionExpression(node, removeDeclareModifier(node.modifiers, typescript), node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body), node, options);
}
...
function visitInterfaceDeclaration$4(options) {
    ...
    if (!hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateInterfaceDeclaration(node, node.decorators, removeDeclareModifier(node.modifiers, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitModuleDeclaration$7(options) {
    ...
    if (!hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateModuleDeclaration(node, node.decorators, removeDeclareModifier(node.modifiers, typescript), node.name, node.body), node, options);
}
...
function visitTypeAliasDeclaration$4(options) {
    ...
    if (!hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateTypeAliasDeclaration(node, node.decorators, removeDeclareModifier(node.modifiers, typescript), node.name, node.typeParameters, node.type), node, options);
}

function visitNode$b({ node, ...options }) {
    if (options.typescript.isClassDeclaration(node)) {
        return visitClassDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isClassExpression(node)) {
        return visitClassExpression$4({ ...options, node });
    }
    else if (options.typescript.isFunctionDeclaration(node)) {
        return visitFunctionDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isFunctionExpression(node)) {
        return visitFunctionExpression$4({ ...options, node });
    }
    else if (options.typescript.isEnumDeclaration(node)) {
        return visitEnumDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isInterfaceDeclaration(node)) {
        return visitInterfaceDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isTypeAliasDeclaration(node)) {
        return visitTypeAliasDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$7({ ...options, node });
    }
    else if (options.typescript.isVariableStatement(node)) {
        return visitVariableStatement$4({ ...options, node });
    }
    else {
...
function ensureNoDeclareModifierTransformer(options) {
    ...
    const visitorOptions = {
        ...options,
        childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$b({
            ...visitorOptions,
            node: nextNode
        }), context),
        continuation: (node) => visitNode$b({
            ...visitorOptions,
            node
        })
    };
...
function visitImportDeclaration$4(options) {
    ...
    return [
        factory.updateImportDeclaration(node, node.decorators, node.modifiers, factory.updateImportClause(node.importClause, first.importClause.isTypeOnly, first.importClause.name, first.importClause.namedBindings), node.moduleSpecifier, node.assertClause),
        ...other
    ];
}
...
function visitExportDeclaration$5(options) {
    ...
    const replacements = options.preserveExportedModuleIfNeeded((_a = node.moduleSpecifier) === null || _a === void 0 ? void 0 : _a.text, node.isTypeOnly);
    if (replacements == null || replacements.length === 0)
        return undefined;
    const [first, ...other] = replacements;
    let exportClause;
...
function visitNode$a({ node, ...options }) {
    if (options.typescript.isImportDeclaration(node)) {
        return visitImportDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isExportDeclaration(node)) {
        return visitExportDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isExportAssignment(node)) {
        return visitExportAssignment$2({ ...options, node });
    }
    else {
        // Only consider root-level statements here
        return node;
...
function getMergedImportDeclarationsForModules(options) {
    ...
    for (const [module, names] of defaultImportsFromModulesMap) {
        ...
        for (const name of names) {
            importDeclarationsForModule.push(preserveParents(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, factory.createIdentifier(name), undefined), factory.createStringLiteral(ensureHasLeadingDotAndPosix(module))), { typescript }));
        }
    }
    ...
    for (const [module, names] of namespaceImportsFromModulesMap) {
        ...
        for (const name of names) {
            importDeclarationsForModule.push(preserveParents(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamespaceImport(factory.createIdentifier(name))), factory.createStringLiteral(ensureHasLeadingDotAndPosix(module))), { typescript }));
        }
    }
    ...
    for (const [module, collections] of namedImportsFromModulesMap) {
        ...
        for (const collection of collections) {
            ...
            if (collection.length < 1)
                continue;
            importDeclarationsForModule.push(preserveParents(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports(collection.map(record => record.propertyName !== record.alias
                ? factory.createImportSpecifier(false, factory.createIdentifier(record.propertyName), factory.createIdentifier(record.alias))
                : factory.createImportSpecifier(false, undefined, factory.createIdentifier(record.alias))))), factory.createStringLiteral(ensureHasLeadingDotAndPosix(module))), { typescript }));
        }
    }
 ...
 */
function getMergedExportDeclarationsForModules(options) {
    var _a;
    ...
    const moduleSpecifierToExportedBindings = new Map();
    ...
    for (const exportDeclaration of exports) {
        if (Boolean(exportDeclaration.isTypeOnly) !== Boolean(isTypeOnly))
            continue;
        // If the ModuleSpecifier is given and it isn't a string literal, leave it as it is
    ...
    for (const [specifier, exportedBindings] of moduleSpecifierToAliasedExportedBindings) {
        ...
        const inputBindingsForSpecifier = inputBindings === null || inputBindings === void 0 ? void 0 : inputBindings.get(specifier);
        ...
        for (const [propertyName, aliases] of exportedBindings) {
            for (const alias of aliases) {
                ...
                if (bindings.has(alias) || Boolean(inputBindingsForSpecifier === null || inputBindingsForSpecifier === void 0 ? void 0 : inputBindingsForSpecifier.has(alias)))
                    continue;
                bindings.add(alias);
                if (propertyName === alias) {
        ...
        exportDeclarationsForModule.push(preserveParents(factory.createExportDeclaration(undefined, undefined, isTypeOnly, factory.createNamedExports(exportSpecifiers), specifier == null ? undefined : factory.createStringLiteral(ensureHasLeadingDotAndPosix(specifier))), { typescript }));
    }
    ...
    for (const [specifier, names] of namedNamespaceExportsFromModulesMap) {
        ...
        for (const name of names) {
            exportDeclarationsForModule.push(preserveParents(factory.createExportDeclaration(undefined, undefined, isTypeOnly, factory.createNamespaceExport(factory.createIdentifier(name)), factory.createStringLiteral(ensureHasLeadingDotAndPosix(specifier))), { typescript }));
        }
    }
    return {
...
function nodeHasSupportedExtension(node, typescript, extensions) {
    ...
    if (node.moduleSpecifier == null || !typescript.isStringLiteralLike(node.moduleSpecifier))
        return true;
    return path.extname(node.moduleSpecifier.text) === "" || extensions.has(path.extname(node.moduleSpecifier.text));
}
...
function statementMerger({ markAsModuleIfNeeded }) {
    return options => {
        ...
        const includedTypeOnlyExportedModules = new Set();
        // Prepare some VisitorOptions
        const visitorOptions = {
            ...options,
            preserveImportedModuleIfNeeded(module) {
                if (includedImportedModules.has(module))
                    return undefined;
                includedImportedModules.add(module);
                return mergedImports.get(module);
            },
            preserveExportedModuleIfNeeded(module, typeOnly) {
                ...
                if (selectedIncludedExportedModules.has(module))
                    return undefined;
                selectedIncludedExportedModules.add(module);
                return selectedMergedExports.get(module);
            },
            childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$a({
                ...visitorOptions,
                node: nextNode
            }), context),
            continuation: (node) => visitNode$a({
                ...visitorOptions,
                node
            })
        };
        ...
        const importExportWithSupportedExtensionsCount = importDeclarationsWithSupportedExtensions.length + exportDeclarationsWithSupportedExtensions.length + statementsWithExportModifier.length;
        result = preserveMeta(factory.updateSourceFile(result, [
            ...importDeclarations,
            ...otherStatements,
            ...exportDeclarations,
...
function ensureNonreservedWord(word) {
...
function isIdentifierFree(lexicalEnvironment, identifier, originalSourceFileName, isInternalAlias = false) {
...
function visitExportDeclaration$4(options) {
    ...
    if (node.exportClause == null || ((_a = typescript.isNamespaceExport) === null || _a === void 0 ? void 0 : _a.call(typescript, node.exportClause))) {
        ...
        const exportedBindings = [...((_e = (_d = (_c = resolvedSourceFile.symbol) === null || _c === void 0 ? void 0 : _c.exports) === null || _d === void 0 ? void 0 : _d.keys()) !== null && _e !== void 0 ? _e : [])].map(binding => binding !== "default" && isIdentifierFree(lexicalEnvironment, binding, originalSourceFile.fileName)
            ? [binding, binding]
            : [binding, generateUniqueBinding(lexicalEnvironment, binding === "default" ? "_default" : binding)]);
        const namedImports = factory.createNamedImports(exportedBindings.map(([name, deconflictedName]) => factory.createImportSpecifier(false, name === deconflictedName ? undefined : factory.createIdentifier(name), factory.createIdentifier(deconflictedName))));
        intentToAddImportDeclaration(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, namedImports), factory.createStringLiteral(node.moduleSpecifier.text)));
        const namedExports = factory.createNamedExports(exportedBindings.map(([name, deconflictedName]) => factory.createExportSpecifier(false, name === deconflictedName ? undefined : factory.createIdentifier(deconflictedName), factory.createIdentifier(name))));
...
function visitNode$9({ node, ...options }) {
    if (options.typescript.isImportDeclaration(node)) {
        return visitImportDeclaration$3({ ...options, node });
    }
    else if (options.typescript.isExportDeclaration(node)) {
        return visitExportDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$6({ ...options, node });
    }
    else {
        // Only consider root-level statements here
        return node;
...
function inlineNamespaceModuleBlockTransformer({ intentToAddImportDeclaration, intentToAddModuleDeclaration }) {
    return options => {
        const { typescript, context, sourceFile, pluginOptions, printer } = options;
        ...
        const visitorOptions = {
            ...
            childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$9({
                ...visitorOptions,
                node: nextNode
            }), context),
            continuation: (node) => visitNode$9({
                ...visitorOptions,
                node
            })
        };
    ...
    for (const symbol of exportedSymbols) {
        ...
        if ("isNamespaceExport" in symbol) {
            ...
            if (matchingSourceFile == null && generatedModuleSpecifier != null) {
                exportDeclarations.push(preserveParents(factory.createExportDeclaration(undefined, undefined, false, undefined, factory.createStringLiteral(generatedModuleSpecifier)), { typescript }));
            }
        ...
        // Otherwise, we can just add an ExportDeclaration with an ExportSpecifier
            ...
            const exportSpecifier = factory.createExportSpecifier(false, symbol.propertyName.text === symbol.name.text ? undefined : factory.createIdentifier(symbol.propertyName.text), factory.createIdentifier(symbol.name.text));
            exportDeclarations.push(preserveParents(factory.createExportDeclaration(undefined, undefined, symbol.isTypeOnly, factory.createNamedExports([exportSpecifier]), symbol.moduleSpecifier == null || generatedModuleSpecifier == null || matchingSourceFile != null ? undefined : factory.createStringLiteral(generatedModuleSpecifier)), { typescript }));
...
function visitExportDeclaration$3(options) {
    ...
    if (matchingSourceFile == null) {
        ...
        return preserveMeta(factory.updateExportDeclaration(contResult, contResult.decorators, contResult.modifiers, contResult.isTypeOnly, contResult.exportClause, factory.createStringLiteral(updatedModuleSpecifier), contResult.assertClause), contResult, options);
    }
    ...
    // so we can potentially avoid inlining the same namespace multiple times
        ...
        const existingInlinedModuleDeclarationName = (updatedModuleSpecifier !== null && updatedModuleSpecifier !== void 0 ? updatedModuleSpecifier : moduleSpecifier) == null ? undefined : options.getNameForInlinedModuleDeclaration(updatedModuleSpecifier !== null && updatedModuleSpecifier !== void 0 ? updatedModuleSpecifier : moduleSpecifier);
        if (existingInlinedModuleDeclarationName == null) {
            ...
            options.prependNodes(...importDeclarations.map(importDeclaration => preserveParents(importDeclaration, options)), ...moduleDeclarations.map(moduleDeclaration => preserveParents(moduleDeclaration, options)), preserveParents(factory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, factory, typescript), factory.createIdentifier(contResult.exportClause.name.text), moduleBlock, typescript.NodeFlags.Namespace), options), preserveParents(factory.createExportDeclaration(undefined, undefined, false, factory.createNamedExports([factory.createExportSpecifier(false, undefined, factory.createIdentifier(contResult.exportClause.name.text))]), undefined), options));
            options.markModuleDeclarationAsInlined(updatedModuleSpecifier !== null && updatedModuleSpecifier !== void 0 ? updatedModuleSpecifier : moduleSpecifier, contResult.exportClause.name.text);
        }
        else {
            options.prependNodes(preserveParents(factory.createExportDeclaration(undefined, undefined, false, factory.createNamedExports([
    ...
    return preserveMeta(factory.updateExportDeclaration(contResult, contResult.decorators, contResult.modifiers, contResult.isTypeOnly, contResult.exportClause, undefined, contResult.assertClause), contResult, options);
}
...
function visitImportTypeNode$1(options) {
    ...
    if (matchingSourceFile == null) {
        ...
        return generatedModuleSpecifier == null
            ? contResult
            : preserveMeta(factory.updateImportTypeNode(contResult, factory.createLiteralTypeNode(factory.createStringLiteral(generatedModuleSpecifier)), contResult.qualifier, contResult.typeArguments, contResult.isTypeOf), node, options);
    }
    let returnNode;
    ...
    if (contResult.qualifier == null) {
        ...
        options.prependNodes(...importDeclarations.map(importDeclaration => preserveParents(importDeclaration, options)), ...moduleDeclarations.map(moduleDeclaration => preserveParents(moduleDeclaration, options)), preserveParents(factory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, factory, typescript), factory.createIdentifier(namespaceName), moduleBlock, typescript.NodeFlags.Namespace), options));
...
function visitImportDeclaration$2(options) {
    ...
    return preserveMeta(factory.updateImportDeclaration(contResult, contResult.decorators, contResult.modifiers, contResult.importClause, factory.createStringLiteral(updatedModuleSpecifier), contResult.assertClause), contResult, options);
}
...
function getAliasedDeclarationFromSymbol(symbol, typeChecker) {
    ...
    try {
        ...
        if (aliasedDeclaration != null && (aliasedDeclaration.valueDeclaration != null || (aliasedDeclaration.declarations != null && aliasedDeclaration.declarations.length > 0))) {
            valueDeclaration = (aliasedDeclaration.valueDeclaration != null ? aliasedDeclaration.valueDeclaration : symbol.declarations != null ? (_a = aliasedDeclaration.declarations) === null || _a === void 0 ? void 0 : _a[0] : undefined);
        }
    }
    catch { }
    return valueDeclaration;
...
function markAsInternalAlias(node, typescript) {
    getOriginalNode(node, typescript)._internalAlias = true;
}
function isNodeInternalAlias(node, typescript) {
    return getOriginalNode(node, typescript)._internalAlias === true;
}

function createAliasedBinding({ factory, typescript, lexicalEnvironment, name, node, propertyName, typeChecker }) {
    ...
    switch (declaration === null || declaration === void 0 ? void 0 : declaration.kind) {
        ...
        case typescript.SyntaxKind.ClassExpression: {
            const moduleDeclaration = factory.createModuleDeclaration(undefined, undefined, factory.createIdentifier(moduleBinding), factory.createModuleBlock([
                factory.createExportDeclaration(undefined, undefined, false, factory.createNamedExports([factory.createExportSpecifier(false, undefined, factory.createIdentifier(propertyName))]))
            ]));
            const importEqualsDeclaration = factory.createImportEqualsDeclaration(undefined, undefined, false, factory.createIdentifier(name), factory.createQualifiedName(factory.createIdentifier(moduleBinding), factory.createIdentifier(propertyName)));
        ...
        case typescript.SyntaxKind.ExportAssignment: {
            const variableStatement = factory.createVariableStatement(ensureHasDeclareModifier(undefined, factory, typescript), factory.createVariableDeclarationList([factory.createVariableDeclaration(factory.createIdentifier(name), undefined, factory.createTypeQueryNode(factory.createIdentifier(propertyName)))], typescript.NodeFlags.Const));
            markAsInternalAlias(variableStatement, typescript);
            return [preserveParents(variableStatement, { typescript })];
        }
        default: {
            const alias = propertyName === "_default"
                ? factory.createVariableStatement(ensureHasDeclareModifier(undefined, factory, typescript), factory.createVariableDeclarationList([factory.createVariableDeclaration(factory.createIdentifier(name), undefined, factory.createTypeQueryNode(factory.createIdentifier(propertyName)))], typescript.NodeFlags.Const))
                : factory.createTypeAliasDeclaration(undefined, undefined, factory.createIdentifier(name), undefined, factory.createTypeReferenceNode(factory.createIdentifier(propertyName), undefined));
...
function visitImportSpecifier$1(options) {
    ...
    if (exportedSymbol != null) {
        ...
        if (exportedSymbol.moduleSpecifier != null &&
            ...
            options.prependNodes(preserveParents(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports([
                factory.createImportSpecifier(false, propertyName.text === "default"
                    ? factory.createIdentifier("default")
                    : exportedSymbol.propertyName.text === contResult.name.text
                        ? undefined
                        : factory.createIdentifier(exportedSymbol.propertyName.text), factory.createIdentifier(contResult.name.text))
            ])), factory.createStringLiteral(generatedModuleSpecifier)), options));
        }
            ...
            options.prependNodes(...createAliasedBinding({
                ...options,
                node: declaration,
...
function visitExportSpecifier(options) {
    ...
    const namedExportedSymbol = propertyName.text === "default"
        ? locateExportedSymbolForSourceFile({ defaultExport: true }, { ...options, sourceFile: payload.matchingSourceFile.fileName })
        : (_b = locateExportedSymbolForSourceFile({ defaultExport: false, name: propertyName.text }, { ...options, sourceFile: payload.matchingSourceFile.fileName })) !== null && _b !== void 0 ? _b : locateExportedSymbolForSourceFile({ namespaceExport: true }, { ...options, sourceFile: payload.matchingSourceFile.fileName });
    if (namedExportedSymbol != null) {
        ...
        if (namedExportedSymbol.moduleSpecifier != null &&
            ...
            options.prependNodes(preserveParents(factory.createExportDeclaration(undefined, undefined, payload.isTypeOnly, factory.createNamedExports([
                factory.createExportSpecifier(false, propertyName.text === "default"
                    ? "default"
                    : !("propertyName" in namedExportedSymbol) || namedExportedSymbol.propertyName == null || namedExportedSymbol.propertyName.text === contResult.name.text
                        ? undefined
                        : factory.createIdentifier(namedExportedSymbol.propertyName.text), factory.createIdentifier(contResult.name.text))
            ]), factory.createStringLiteral(generatedModuleSpecifier)), options));
            return undefined;
        }
            ...
            const newPropertyName = !("propertyName" in namedExportedSymbol) || namedExportedSymbol.propertyName == null || namedExportedSymbol.propertyName.text === contResult.name.text
                ? undefined
                : ensureNonreservedWord(namedExportedSymbol.propertyName.text);
            if (newPropertyName !== ((_c = contResult.propertyName) === null || _c === void 0 ? void 0 : _c.text)) {
                return preserveMeta(factory.updateExportSpecifier(contResult, false, newPropertyName == null ? undefined : factory.createIdentifier(newPropertyName), factory.createIdentifier(contResult.name.text)), contResult, options);
            }
        }
    }
...
function visitImportClause$1(options) {
    ...
    if (defaultExportedSymbol != null) {
        ...
        if (defaultExportedSymbol.moduleSpecifier != null &&
            ...
            options.prependNodes(preserveParents(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, factory.createIdentifier(contResult.name.text), undefined), factory.createStringLiteral(generatedModuleSpecifier)), { typescript }));
        }
...
function visitNamespaceImport$1(options) {
    ...
    if (payload.moduleSpecifier == null)
        return options.childContinuation(node, undefined);
    const contResult = options.childContinuation(node, undefined);
    const importedSymbol = getImportedSymbolFromNamespaceImport(contResult, payload.moduleSpecifier);
    // If no SourceFile was resolved, preserve the ImportSpecifier as-is, unless it is already included in the chunk
    if (payload.matchingSourceFile == null) {
        ...
        if (existingInlinedModuleDeclarationName == null) {
            ...
            // Otherwise, prepend the nodes for the SourceFile in a namespace declaration
            options.prependNodes(...importDeclarations.map(importDeclaration => preserveParents(importDeclaration, options)), ...moduleDeclarations.map(moduleDeclaration => preserveParents(moduleDeclaration, options)), preserveParents(factory.createModuleDeclaration(undefined, ensureHasDeclareModifier(undefined, factory, typescript), factory.createIdentifier(contResult.name.text), moduleBlock, typescript.NodeFlags.Namespace), options));
            options.markModuleDeclarationAsInlined(payload.moduleSpecifier, contResult.name.text);
        }
        else {
            options.prependNodes(preserveParents(factory.createImportEqualsDeclaration(undefined, undefined, false, factory.createIdentifier(contResult.name.text), factory.createIdentifier(existingInlinedModuleDeclarationName)), { typescript }));
        }
    }
...
function visitNode$8({ node, ...options }) {
    if (options.typescript.isSourceFile(node)) {
        return visitSourceFile({ ...options, node });
    }
    else if (options.typescript.isExportDeclaration(node)) {
        return visitExportDeclaration$3({ ...options, node });
    }
    else if (options.typescript.isImportDeclaration(node)) {
        return visitImportDeclaration$2({ ...options, node });
    }
    else if (options.typescript.isImportTypeNode(node)) {
        return visitImportTypeNode$1({ ...options, node });
    }
    else if (options.typescript.isImportClause(node)) {
        return visitImportClause$1({ ...options, node });
    }
    else if (options.typescript.isNamespaceImport(node)) {
        return visitNamespaceImport$1({ ...options, node });
    }
    else if (options.typescript.isImportSpecifier(node)) {
        return visitImportSpecifier$1({ ...options, node });
    }
    else if (options.typescript.isExportSpecifier(node)) {
        return visitExportSpecifier({ ...options, node });
    }
    else {
...
function findMatchingImportedSymbol(importedSymbol, importedSymbols, { loose = false } = {}) {
...
function visitClassDeclaration$3(options) {
    ...
    if (!hasExportModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateClassDeclaration(node, node.decorators, removeExportModifier(node.modifiers, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitClassExpression$3(options) {
    ...
    if (!hasExportModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateClassExpression(node, node.decorators, removeExportModifier(node.modifiers, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitFunctionDeclaration$3(options) {
    ...
    if (!hasExportModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateFunctionDeclaration(node, node.decorators, removeExportModifier(node.modifiers, typescript), node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body), node, options);
}
...
function visitFunctionExpression$3(options) {
    ...
    if (!hasExportModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateFunctionExpression(node, removeExportModifier(node.modifiers, typescript), node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body), node, options);
}
...
function visitInterfaceDeclaration$3(options) {
    ...
    if (!hasExportModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateInterfaceDeclaration(node, node.decorators, removeExportModifier(node.modifiers, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitTypeAliasDeclaration$3(options) {
    ...
    if (!hasExportModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateTypeAliasDeclaration(node, node.decorators, removeExportModifier(node.modifiers, typescript), node.name, node.typeParameters, node.type), node, options);
}

function visitNode$7({ node, ...options }) {
    if (options.typescript.isClassDeclaration(node)) {
        return visitClassDeclaration$3({ ...options, node });
    }
    else if (options.typescript.isClassExpression(node)) {
        return visitClassExpression$3({ ...options, node });
    }
    else if (options.typescript.isFunctionDeclaration(node)) {
        return visitFunctionDeclaration$3({ ...options, node });
    }
    else if (options.typescript.isFunctionExpression(node)) {
        return visitFunctionExpression$3({ ...options, node });
    }
    else if (options.typescript.isEnumDeclaration(node)) {
        return visitEnumDeclaration$3({ ...options, node });
    }
    else if (options.typescript.isInterfaceDeclaration(node)) {
        return visitInterfaceDeclaration$3({ ...options, node });
    }
    else if (options.typescript.isTypeAliasDeclaration(node)) {
        return visitTypeAliasDeclaration$3({ ...options, node });
    }
    else if (options.typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$5({ ...options, node });
    }
    else if (options.typescript.isVariableStatement(node)) {
        return visitVariableStatement$3({ ...options, node });
    }
    else {
        // Only consider root-level statements here
        return node;
...
function ensureNoExportModifierTransformer(options) {
    ...
    const visitorOptions = {
        ...options,
        childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$7({
            ...visitorOptions,
            node: nextNode
        }), context),
        continuation: (node) => visitNode$7({
            ...visitorOptions,
            node
        })
    };
...
function visitNode$6({ node, ...options }) {
    if (options.typescript.isExportDeclaration(node)) {
        return visitExportDeclaration$2({ ...options, node });
    }
    else if (options.typescript.isExportAssignment(node)) {
        return visitExportAssignment$1({ ...options, node });
    }
    else {
        // Preserve the node
        return node;
...
function noExportDeclarationTransformer({ preserveAliasedExports = false, preserveExportsWithModuleSpecifiers = false } = {}) {
    return options => {
        ...
        const visitorOptions = {
            ...
            childContinuation: (node) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$6({
                ...visitorOptions,
                node: nextNode
            })), context),
            continuation: (node) => nodePlacementQueue.wrapVisitResult(visitNode$6({
                ...visitorOptions,
                node
            }))
        };
...
function moduleMerger(...transformers) {
    return options => {
        ...
        const visitorOptions = {
            ...
            transformers,
            payload: undefined,
            childContinuation: (node, payload) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$8({
                ...visitorOptions,
                payload,
                node: nextNode
            })), context),
            continuation: (node, payload) => nodePlacementQueue.wrapVisitResult(visitNode$8({
                ...visitorOptions,
                payload,
                node
            })),
            getNameForInlinedModuleDeclaration(moduleSpecifier) {
                var _a;
                const name = inlinedModules.get(moduleSpecifier);
                if (name == null)
                    return undefined;
                return (_a = getBindingFromLexicalEnvironment(options.lexicalEnvironment, name)) !== null && _a !== void 0 ? _a : name;
            },
            markModuleDeclarationAsInlined(moduleSpecifier, name) {
                inlinedModules.set(moduleSpecifier, name);
            },
            shouldPreserveImportedSymbol(importedSymbol) {
            ...
            includeSourceFile(sourceFileToInclude, { allowDuplicate = false, allowExports = options.otherEntrySourceFilesForChunk.some(otherEntrySourceFileForChunk => otherEntrySourceFileForChunk.fileName === sourceFileToInclude.fileName), transformers: extraTransformers = [], ...otherOptions } = {}) {
        ...
        if (missingPrependNodes.length > 0 || missingAppendNodes.length > 0) {
            result = preserveMeta(factory.updateSourceFile(result, [...missingPrependNodes, ...result.statements, ...missingAppendNodes], result.isDeclarationFile, result.referencedFiles, result.typeReferenceDirectives, result.hasNoDefaultLib, result.libReferenceDirectives), result, options);
        }
        transformationLog === null || transformationLog === void 0 ? void 0 : transformationLog.finish(result);
        fullBenchmark === null || fullBenchmark === void 0 ? void 0 : fullBenchmark.finish();
...
function deconflictClassDeclaration(options) {
    ...
    return preserveMeta(factory.updateClassDeclaration(node, node.decorators, node.modifiers, nameContResult, typeParametersContResult, heritageClausesContResult, membersContResult), node, options);
}
...
function deconflictClassExpression(options) {
    ...
    return preserveMeta(factory.updateClassExpression(node, node.decorators, node.modifiers, nameContResult, typeParametersContResult, heritageClausesContResult, membersContResult), node, options);
}
...
function deconflictFunctionDeclaration(options) {
    ...
    return preserveMeta(factory.updateFunctionDeclaration(node, node.decorators, node.modifiers, node.asteriskToken, nameContResult, typeParametersContResult, parametersContResult, typeContResult, bodyContResult), node, options);
}
...
function deconflictFunctionExpression(options) {
    ...
    return preserveMeta(factory.updateFunctionExpression(node, node.modifiers, node.asteriskToken, nameContResult, typeParametersContResult, parametersContResult, typeContResult, bodyContResult), node, options);
}
...
function deconflictGetAccessorDeclaration(options) {
    ...
    return preserveMeta(factory.updateGetAccessorDeclaration(node, node.decorators, node.modifiers, nameContResult, parametersContResult, typeContResult, bodyContResult), node, options);
}
...
function deconflictIdentifier(options) {
    ...
    const id = getIdForNode({ ...options, node: (_a = getBestDeclaration(options)) !== null && _a !== void 0 ? _a : node });
...
function deconflictInterfaceDeclaration(options) {
    ...
    return preserveMeta(factory.updateInterfaceDeclaration(node, node.decorators, node.modifiers, nameContResult, typeParametersContResult, heritageClausesContResult, membersContResult), node, options);
}
...
function deconflictMappedTypeNode(options) {
    ...
    return preserveMeta(factory.updateMappedTypeNode(node, node.readonlyToken, typeParameterContResult, nameTypeContResult, node.questionToken, typeContResult, membersContResult), node, options);
}
...
function deconflictMethodDeclaration(options) {
    ...
    return preserveMeta(factory.updateMethodDeclaration(node, node.decorators, node.modifiers, node.asteriskToken, nameContResult, node.questionToken, typeParametersContResult, parametersContResult, typeContResult, bodyContResult), node, options);
}
...
function deconflictMethodSignature(options) {
    ...
    const nextContinuationOptions = { lexicalEnvironment: cloneLexicalEnvironment(lexicalEnvironment) };
    const typeParametersContResult = node.typeParameters == null ? undefined : factory.createNodeArray(node.typeParameters.map(typeParameter => continuation(typeParameter, nextContinuationOptions)));
    ...
    return preserveMeta(factory.updateMethodSignature(node, node.modifiers, nameContResult, node.questionToken, typeParametersContResult, parametersContResult, typeContResult), node, options);
}
...
function deconflictParameterDeclaration(options) {
    ...
    return preserveMeta(factory.updateParameterDeclaration(node, node.decorators, node.modifiers, node.dotDotDotToken, nameContResult, node.questionToken, typeContResult, initializerContResult), node, options);
}
...
function deconflictPropertyDeclaration(options) {
    ...
    return preserveMeta(factory.updatePropertyDeclaration(node, node.decorators, node.modifiers, nameContResult, (_a = node.questionToken) !== null && _a !== void 0 ? _a : node.exclamationToken, typeContResult, initializerContResult), node, options);
}
...
function deconflictVariableDeclaration(options) {
    ...
    const upperNode = variableDeclarationList == null ? node : (_a = getParentNode(variableDeclarationList)) !== null && _a !== void 0 ? _a : variableDeclarationList;
    if (typescript.isIdentifier(node.name)) {
        const id = getIdForNode(options);
        const originalSourceFile = getOriginalSourceFile(node, sourceFile, typescript);
        if (id != null)
...
function deconflictCallSignatureDeclaration(options) {
    ...
    const parametersContResult = factory.createNodeArray(node.parameters.map(parameter => continuation(parameter, nextContinuationOptions)));
    const typeContResult = node.type == null ? undefined : continuation(node.type, nextContinuationOptions);
    const isIdentical = nodeArraysAreEqual(typeParametersContResult, node.typeParameters) && nodeArraysAreEqual(parametersContResult, node.parameters) && typeContResult === node.type;
    if (isIdentical) {
        return node;
    }
 ...
 * Deconflicts the given ImportClause.
 */
function deconflictImportEqualsDeclaration(options) {
 ...
 */
function deconflictNode({ node, ...options }) {
    var _a, _b;
    if (options.typescript.isBindingElement(node)) {
    ...
    else if ((_b = (_a = options.typescript).isMappedTypeNode) === null || _b === void 0 ? void 0 : _b.call(_a, node)) {
...
function deconflicter(options) {
    ...
    const visitorOptions = {
        ...options,
        childContinuation: (node, continuationOptions) => typescript.visitEachChild(node, nextNode => deconflictNode({
            ...visitorOptions,
            ...continuationOptions,
            node: nextNode
        }), context),
        continuation: (node, continuationOptions) => deconflictNode({
            ...visitorOptions,
            ...continuationOptions,
            node
        })
...
function visitClassDeclaration$2(options) {
    ...
    if (hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateClassDeclaration(node, node.decorators, ensureHasDeclareModifier(node.modifiers, factory, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitClassExpression$2(options) {
    ...
    if (hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateClassExpression(node, node.decorators, ensureHasDeclareModifier(node.modifiers, factory, typescript), node.name, node.typeParameters, node.heritageClauses, node.members), node, options);
}
...
function visitFunctionDeclaration$2(options) {
    ...
    if (hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateFunctionDeclaration(node, node.decorators, ensureHasDeclareModifier(node.modifiers, factory, typescript), node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body), node, options);
}
...
function visitFunctionExpression$2(options) {
    ...
    if (hasDeclareModifier(node, typescript))
        return node;
    return preserveMeta(factory.updateFunctionExpression(node, ensureHasDeclareModifier(node.modifiers, factory, typescript), node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body), node, options);
}
...
function visitNode$5({ node, ...options }) {
    if (options.typescript.isClassDeclaration(node)) {
        return visitClassDeclaration$2({ ...options, node });
    }
    else if (options.typescript.isClassExpression(node)) {
        return visitClassExpression$2({ ...options, node });
    }
    else if (options.typescript.isFunctionDeclaration(node)) {
        return visitFunctionDeclaration$2({ ...options, node });
    }
    else if (options.typescript.isFunctionExpression(node)) {
        return visitFunctionExpression$2({ ...options, node });
    }
    else if (options.typescript.isEnumDeclaration(node)) {
        return visitEnumDeclaration$2({ ...options, node });
    }
    else if (options.typescript.isInterfaceDeclaration(node)) {
        return visitInterfaceDeclaration$2({ ...options, node });
    }
    else if (options.typescript.isTypeAliasDeclaration(node)) {
        return visitTypeAliasDeclaration$2({ ...options, node });
    }
    else if (options.typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$4({ ...options, node });
    }
    else if (options.typescript.isVariableStatement(node)) {
        return visitVariableStatement$2({ ...options, node });
    }
    else {
        // Only consider root-level statements here
        return node;
...
function ensureDeclareModifierTransformer(options) {
    ...
    const visitorOptions = {
        ...options,
        childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$5({
            ...visitorOptions,
            node: nextNode
        }), context),
        continuation: (node) => visitNode$5({
            ...visitorOptions,
            node
        })
    };
...
function visitNode$4({ node, ...options }) {
    if (options.typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$3({ ...options, node });
    }
    else {
        return options.childContinuation(node);
    }
...
function moduleBlockExtractor(options) {
    ...
    const visitorOptions = {
        ...options,
        childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$4({
            ...visitorOptions,
            node: nextNode
        }), context),
        continuation: (node) => visitNode$4({
            ...visitorOptions,
            node
        })
    };
...
function checkImportEqualsDeclaration({ node, continuation, markIdentifiersAsReferenced }) {
    const referencedIdentifiers = [];
 ...
 */
function checkNode({ node, originalNode, ...options }) {
    var _a, _b;
    ...
    else if ((_b = (_a = options.typescript).isTemplateLiteralTypeSpan) === null || _b === void 0 ? void 0 : _b.call(_a, node)) {
 ...
 */
function isReferenced({ seenNodes = new Set(), ...options }) {
    ...
    // Compute the result
    const result = referencingNodes.length > 0 && referencingNodes.some(referencingNode => isReferenced({ ...options, seenNodes, node: referencingNode, referencedNode: options.node }));
    // Cache the result
...
function collectReferences(options, identifiers) {
    ...
    if (nodeToReferencedIdentifiersCache == null) {
        ...
        const visitorOptions = {
            ...
            markIdentifiersAsReferenced(fromNode, ...referencedIdentifiers) {
            ...
            continuation: (node) => checkNode({ ...visitorOptions, node })
        };
...
function visitClassDeclaration$1({ node, continuation, factory }) {
...
function visitClassExpression$1({ node, continuation, factory }) {
    const nameContinuationResult = node.name == null ? undefined : continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitFunctionDeclaration$1({ node, continuation, factory }) {
    const nameContinuationResult = node.name == null ? undefined : continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
    return node.name === nameContinuationResult
        ? node
        : factory.updateFunctionDeclaration(node, node.decorators, node.modifiers, node.asteriskToken, nameContinuationResult, node.typeParameters, node.parameters, node.type, node.body);
}

function visitFunctionExpression$1({ node, continuation, factory }) {
    const nameContinuationResult = node.name == null ? undefined : continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitEnumDeclaration$1({ node, continuation, factory }) {
...
function visitInterfaceDeclaration$1(options) {
    ...
    return node.name === nameContinuationResult
        ? node
        : preserveMeta(factory.updateInterfaceDeclaration(node, node.decorators, node.modifiers, nameContinuationResult, node.typeParameters, node.heritageClauses, node.members), node, options);
}

function visitTypeAliasDeclaration$1({ node, continuation, factory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitVariableStatement$1({ node, continuation, factory }) {
...
function visitVariableDeclarationList({ node, continuation, factory }) {
...
function visitVariableDeclaration({ node, continuation, factory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitImportDeclaration$1({ node, continuation, factory }) {
    if (node.importClause == null)
        return undefined;
...
function visitImportSpecifier({ node, continuation, factory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitImportClause({ node, continuation, factory }) {
...
function visitNamedImports({ node, continuation, factory }) {
...
function visitNamespaceImport({ node, continuation, factory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitArrayBindingPattern({ node, continuation, factory }) {
...
function visitObjectBindingPattern({ node, continuation, factory }) {
...
function visitBindingElement({ node, continuation, factory }) {
    const nameContinuationResult = continuation(node.name);
    if (nameContinuationResult == null) {
        return undefined;
    }
...
function visitNode$3(options) {
    ...
    if (typescript.isClassDeclaration(node)) {
        return visitClassDeclaration$1({ ...options, node });
    }
    else if (typescript.isClassExpression(node)) {
        return visitClassExpression$1({ ...options, node });
    }
    else if (typescript.isFunctionDeclaration(node)) {
        return visitFunctionDeclaration$1({ ...options, node });
    }
    else if (typescript.isFunctionExpression(node)) {
        return visitFunctionExpression$1({ ...options, node });
    }
    else if (typescript.isEnumDeclaration(node)) {
        return visitEnumDeclaration$1({ ...options, node });
    }
    else if (typescript.isInterfaceDeclaration(node)) {
        return visitInterfaceDeclaration$1({ ...options, node });
    }
    else if (typescript.isTypeAliasDeclaration(node)) {
        return visitTypeAliasDeclaration$1({ ...options, node });
    }
    else if (typescript.isModuleDeclaration(node)) {
        return visitModuleDeclaration$2({ ...options, node });
    }
    else if (typescript.isExportDeclaration(node)) {
        return visitExportDeclaration$1({ ...options, node });
    }
        ...
        return visitVariableStatement$1({ ...options, node });
    }
        ...
        return visitImportDeclaration$1({ ...options, node });
    }
        ...
        return visitIdentifier$1({ ...options, node });
    }
    else {
...
function treeShaker(options) {
    ...
    const visitorOptions = {
        ...
        isReferenced: (node) => isReferenced({ ...visitorOptions, node }),
        continuation: (node) => visitNode$3({ ...visitorOptions, node })
    };
...
function visitClassDeclaration(options) {
    ...
    if (node.name != null && nameText === node.name.text) {
        ...
        returnNode = preserveMeta(factory.updateClassDeclaration(node, node.decorators, node.modifiers, factory.createIdentifier(nameText), node.typeParameters, node.heritageClauses, node.members), node, options);
    }
...
function visitClassExpression(options) {
    ...
    if (node.name != null && nameText === node.name.text) {
        ...
        returnNode = preserveMeta(factory.updateClassExpression(node, node.decorators, node.modifiers, factory.createIdentifier(nameText), node.typeParameters, node.heritageClauses, node.members), node, options);
    }
    const propertyName = (_a = exportSpecifier.propertyName) !== null && _a !== void 0 ? _a : exportSpecifier.name;
    preserveSymbols(propertyName, returnNode, options);
    return returnNode;
...
function visitFunctionDeclaration(options) {
    ...
    if (node.name != null && nameText === node.name.text) {
        ...
        returnNode = preserveMeta(factory.updateFunctionDeclaration(node, node.decorators, node.modifiers, node.asteriskToken, factory.createIdentifier(nameText), node.typeParameters, node.parameters, node.type, node.body), node, options);
    }
    const propertyName = (_a = exportSpecifier.propertyName) !== null && _a !== void 0 ? _a : exportSpecifier.name;
    preserveSymbols(propertyName, returnNode, options);
    return returnNode;
...
function visitFunctionExpression(options) {
    ...
    if (node.name != null && nameText === node.name.text) {
        ...
        returnNode = preserveMeta(factory.updateFunctionExpression(node, node.modifiers, node.asteriskToken, factory.createIdentifier(nameText), node.typeParameters, node.parameters, node.type, node.body), node, options);
    }
    const propertyName = (_a = exportSpecifier.propertyName) !== null && _a !== void 0 ? _a : exportSpecifier.name;
    preserveSymbols(propertyName, returnNode, options);
    return returnNode;
...
function visitNode$2({ node, ...options }) {
    if (options.typescript.isClassDeclaration(node)) {
        ...
        return visitModuleDeclaration$1({ ...options, node });
    }
    else if (options.typescript.isVariableStatement(node)) {
...
function toExportDeclarationTransformer(options) {
    ...
    const visitorOptions = {
        ...options,
        ...nodePlacementQueue,
        childContinuation: (node) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$2({
            ...visitorOptions,
            node: nextNode
        })), context),
        continuation: (node) => nodePlacementQueue.wrapVisitResult(visitNode$2({
            ...visitorOptions,
            node
        }))
    };
    ...
    const [missingPrependNodes, missingAppendNodes] = nodePlacementQueue.flush();
    if (missingPrependNodes.length > 0 || missingAppendNodes.length > 0) {
        result = factory.updateSourceFile(result, [...missingPrependNodes, ...result.statements, ...missingAppendNodes], result.isDeclarationFile, result.referencedFiles, result.typeReferenceDirectives, result.hasNoDefaultLib, result.libReferenceDirectives);
    }
...
function visitNode$1({ node, ...options }) {
...
function normalizeChunkFilename(filename) {
...
class ModuleResolutionHost {
    ...
    getSupportedNonAmbientExtensions() {
        if (this.currentNonAmbientSupportedExtensions == null) {
            this.currentNonAmbientSupportedExtensions = new Set([...this.options.extensions].filter(extension => !AMBIENT_EXTENSIONS.has(extension)));
        }
 ...
 */
function resolveId({ resolveCache, ...options }) {
...
function ensureModuleTransformer({ typescript, factory, sourceFile, extensions }) {
    ...
    if (importDeclarationCount < 1 && exportDeclarationCount < 1 && exportAssignmentCount < 1) {
        return factory.updateSourceFile(sourceFile, [...sourceFile.statements, factory.createExportDeclaration(undefined, undefined, false, factory.createNamedExports([]))], sourceFile.isDeclarationFile, sourceFile.referencedFiles, sourceFile.typeReferenceDirectives, sourceFile.hasNoDefaultLib, sourceFile.libReferenceDirectives);
    }
    return sourceFile;
}
...
function visitModuleDeclaration({ node, ...options }) {
    if (node.body == null)
        return;
    return options.childContinuation(node.body);
}
...
class CompilerHost extends ModuleResolutionHost {
    ...
    clone(compilerOptions, fileNameFilter = () => true, overrides = {}) {
        ...
        }, this.printer, new Map([...this.sourceFiles.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.transformerDiagnostics.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.fileToVersionMap.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.sourceFileToDependenciesMap.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.files.entries()].filter(([p]) => fileNameFilter(p))));
    }
    getSourceFile(fileName, languageVersionOrOptions = this.getScriptTarget(), onError, shouldCreateNewSourceFile) {
        var _a;
        try {
    ...
    addDefaultFileNames() {
        this.getParsedCommandLine().fileNames.forEach(file => {
            ...
            if (!this.getFilter()(path.normalize(fileName)))
                return;
 ...
 */
function listFormat(elements, andOrOr = "and", mapper = element => element) {
    const arr = [...elements];
    if (arr.length === 0)
        return "";
 ...
 */
let swcModule;
async function loadBabel(assertHasPresetTypescript = false) {
    return (babelModule !== null && babelModule !== void 0 ? babelModule : (babelModule = await loadModules("babel", "@babel/core", [
        "@babel/runtime",
...
async function loadSwc() {
    return (swcModule !== null && swcModule !== void 0 ? swcModule : (swcModule = await loadModules("swc", "@swc/core", ["@swc/helpers"])));
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function loadModules(context, coreModule, subModules) {
    const moduleNames = [coreModule, ...(subModules !== null && subModules !== void 0 ? subModules : [])];
    ...
    const results = await Promise.all(moduleNames
    ...
    const rejectedModuleNames = moduleNames.filter((moduleName, index) => {
        ...
        if (!(result.reason instanceof Error))
            return true;
        return ("code" in result.reason &&
            "message" in result.reason &&
            result.reason.code === "MODULE_NOT_FOUND" &&
            result.reason.message.includes(moduleName));
    });
    if (core.status === "rejected" || rejectedModuleNames.length > 0) {
        ...
        throw new ReferenceError(`The following ${context} ${rejectedModuleNames.length === 1 ? "dependency" : "dependencies"} could not be found within your node_modules folder: ${formattedRejectedModuleNames}. Make sure to install ${rejectedModuleNames.length === 1 ? "it" : "them"} if you want to use ${context} for transpilation`);
    }
 ...
 */
function getBabelConfig({ babel, babelConfig, cwd, forcedOptions = {}, defaultOptions = {}, browserslist, phase, hook }) {
    return async (filename, inTypescriptStep = false) => {
        ...
        const combined = {
            ...
            ...options,
            ...otherForcedOptions,
            presets: combineConfigItems(((_a = options.presets) !== null && _a !== void 0 ? _a : []), defaultPresets == null ? undefined : (_c = (_b = babel.loadPartialConfig({ presets: defaultPresets, ...configFileOption })) === null || _b === void 0 ? void 0 : _b.options.presets) !== null && _c !== void 0 ? _c : undefined, forcedPresets == null
                ? undefined
                : (_e = (_d = babel.loadPartialConfig({ presets: forcedPresets, ...configFileOption })) === null || _d === void 0 ? void 0 : _d.options.presets) !== null && _e !== void 0 ? _e : undefined, phase === "chunk", inTypescriptStep),
            plugins: combineConfigItems(((_f = options.plugins) !== null && _f !== void 0 ? _f : []), defaultPlugins == null ? undefined : (_h = (_g = babel.loadPartialConfig({ plugins: defaultPlugins, ...configFileOption })) === null || _g === void 0 ? void 0 : _g.options.plugins) !== null && _h !== void 0 ? _h : undefined, forcedPlugins == null
                ? undefined
                : (_k = (_j = babel.loadPartialConfig({ plugins: forcedPlugins, ...configFileOption })) === null || _j === void 0 ? void 0 : _j.options.plugins) !== null && _k !== void 0 ? _k : undefined, phase === "chunk", inTypescriptStep)
        };
        ...
        const loadedOptions = (_o = babel.loadOptions({ ...combinedOptionsAfterHook, filename, ...configFileOption })) !== null && _o !== void 0 ? _o : undefined;
...
function isBabelConfig(babelConfig) {
    return babelConfig != null && typeof babelConfig !== "string";
}
/**
 * Combines the given two sets of presets
 */
function combineConfigItems(userItems, defaultItems = [], forcedItems = [], inChunkPhase, inTypescriptStep) {
    ...
    return ([
        ...
        .filter(configItem => { var _a, _b; return ((_a = configItem.file) === null || _a === void 0 ? void 0 : _a.resolved) == null || !isBabelPresetTypescript((_b = configItem.file) === null || _b === void 0 ? void 0 : _b.resolved) || inTypescriptStep; }));
}
/**
...
function replaceBabelHelpers(code, filename, target) {
...
function readConfig(config, cwd, fileSystem) {
    if (config == null) {
 ...
 */
function getSwcConfigFactory({ fileSystem, swcConfig, cwd, browserslist, ecmaVersion, phase, typescript, hook, pluginOptions }) {
    ...
    return (filename, initial = false) => {
        ...
        const inputConfig = (_a = inputConfigs.find(config => config.test == null || config.test.some((regex) => regex.test(filename)))) !== null && _a !== void 0 ? _a : inputConfigs[0];
        ...
        const syntax = (_d = (_c = (_b = inputConfig.jsc) === null || _b === void 0 ? void 0 : _b.parser) === null || _c === void 0 ? void 0 : _c.syntax) !== null && _d !== void 0 ? _d : (initial ? "typescript" : "ecmascript");
        ...
        const config = {
            ...
            jsc: {
                ...
                parser: {
                    ...
                    ...(_e = inputConfig.jsc) === null || _e === void 0 ? void 0 : _e.parser
                },
        ...
        return (_f = hook === null || hook === void 0 ? void 0 : hook(config, filename, phase)) !== null && _f !== void 0 ? _f : config;
    };
}
 ...
 */
function inputOptionsAreEqual(a, b) {
    return inputValuesAreEqual(a, b).equal;
}
function inputValuesAreEqual(a, b, path = []) {
    if (a === b || (a == null && b == null))
        ...
        if (aKeys.length !== bKeys.length)
            return { equal: false, path };
        else if (aKeys.some(key => !inputValuesAreEqual(a[key], b[key], [...path, key]).equal)) {
            return { equal: false, path };
        }
        else {
            return { equal: true };
...
function typescriptRollupPlugin(pluginInputOptions = {}) {
    ...
    const filter = (id) => !isSwcHelper(id) && (internalFilter(id) || internalFilter(path.normalize(id)) || internalFilter(path.native.normalize(id)));
    /**
    ...
    const transpileWithBabel = async (fileName, input, initial = false) => {
        var _a;
        ...
        return (transpilationResult === null || transpilationResult === void 0 ? void 0 : transpilationResult.code) == null
            ? undefined
            : {
    ...
    const transpileWithSwc = async (fileName, input, initial = false) => {
        var _a;
     ...
     */
    const transpileWithTypescript = (fileName, input) => {
    ...
    const isFileRelevant = (code, file) => {
    ...
    return {
        ...
        async options(options) {
            ...
            if (rollupInputOptions != null && inputOptionsAreEqual(rollupInputOptions, options))
                return;
            // Re-assign the full input options
            rollupInputOptions = options;
            const multiEntryPlugin = (_a = options.plugins) === null || _a === void 0 ? void 0 : _a.find(plugin => plugin != null && typeof plugin !== "boolean" && plugin.name === "multi-entry");
            ...
            SUPPORTED_EXTENSIONS = getSupportedExtensions(Boolean(parsedCommandLineResult.parsedCommandLine.options.allowJs), Boolean(parsedCommandLineResult.parsedCommandLine.options.resolveJsonModule), typescript);
            ...
            if (transpilerOptions.otherSyntax === "babel") {
                ...
                const transpilationResult = await babel.transformAsync(updatedCode, {
                    ...
                    ...(updatedMap == null
                        ? {}
                        : {
                            inputSourceMap: { ...updatedMap, file: (_a = updatedMap.file) !== null && _a !== void 0 ? _a : "" }
                        })
                });
                ...
                return {
                    code: transpilationResult.code,
                    map: (_c = transpilationResult.map) !== null && _c !== void 0 ? _c : undefined
                };
            }
            else {
                return updatedSourceDescription !== null && updatedSourceDescription !== void 0 ? updatedSourceDescription : null;
        ...
        async transform(code, file) {
            ...
            if (!relevant)
                return undefined;
            let sourceDescription = { code, map: undefined };
            ...
            if (isUsingTranspiler("babel", transpilerOptions) && isBabelHelper(normalizedFile)) {
                sourceDescription = (_a = replaceBabelHelpers(code, normalizedFile, "esm")) !== null && _a !== void 0 ? _a : sourceDescription;
            }
            ...
            switch (transpilerOptions.typescriptSyntax) {
                case "typescript": {
                    if (isSupportedByCompilerHost) {
                        sourceDescription = (_b = emitFile(file)) !== null && _b !== void 0 ? _b : sourceDescription;
                    }
                    break;
                }
                case "babel": {
                    sourceDescription = (_c = (await transpileWithBabel(file, sourceDescription, true))) !== null && _c !== void 0 ? _c : sourceDescription;
                    break;
                }
                case "swc": {
                    sourceDescription = (_d = (await transpileWithSwc(file, sourceDescription, true))) !== null && _d !== void 0 ? _d : sourceDescription;
                    break;
                }
            }
            ...
            if (transpilerOptions.otherSyntax === transpilerOptions.typescriptSyntax) {
                ...
                switch (transpilerOptions.otherSyntax) {
                    case "typescript": {
                        return (_e = transpileWithTypescript(file, sourceDescription)) !== null && _e !== void 0 ? _e : sourceDescription;
                    }
                    case "babel": {
                        return (_f = (await transpileWithBabel(file, sourceDescription))) !== null && _f !== void 0 ? _f : sourceDescription;
                    }
                    case "swc": {
                        return (_g = (await transpileWithSwc(file, sourceDescription))) !== null && _g !== void 0 ? _g : sourceDescription;
                    }
                }
            }
        },
        ...
        generateBundle(outputOptions, bundle) {
            ...
            if (inputCache != null) {
                for (const module of inputCache.modules) {
                    ...
                    if (host.has(normalizedFile) || !isFileRelevant(module.originalCode, normalizedFile).isSupportedByCompilerHost)
                        continue;
...
export { typescriptRollupPlugin as default };
