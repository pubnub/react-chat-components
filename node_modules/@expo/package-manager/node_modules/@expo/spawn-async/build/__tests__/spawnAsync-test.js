...
it(`receives output from completed processes`, async () => {
    let result = await (0, spawnAsync_1.default)('echo', ['hi']);
...
it(`throws errors when processes return non-zero exit codes`, async () => {
    let didThrow = false;
    try {
        await (0, spawnAsync_1.default)('false');
    }
    catch (e) {
        didThrow = true;
...
it(`returns when processes are killed with signals with non-zero exit codes`, async () => {
    let didThrow = false;
    try {
        await (0, spawnAsync_1.default)(path_1.default.join(__dirname, 'signal-self.sh'));
    }
    catch (e) {
        didThrow = true;
        expect(typeof e.pid).toBe('number');
...
it(`throws errors when processes don't exist`, async () => {
    let didThrow = false;
    try {
        await (0, spawnAsync_1.default)('nonexistent-program');
    }
    catch (e) {
        didThrow = true;
...
it(`exposes the child process through a property named "child"`, async () => {
    let spawnTask = (0, spawnAsync_1.default)('echo', ['hi']);
...
it(`runs extra error listeners added to the child process when there is an error`, async () => {
    let spawnTask = (0, spawnAsync_1.default)('nonexistent-program');
...
it(`returns empty strings when ignoring stdio`, async () => {
    let result = await (0, spawnAsync_1.default)('echo', ['hi'], { ignoreStdio: true });
    expect(typeof result.pid).toBe('number');
...
it(`returns even if stdout is open when ignoring stdio`, async () => {
    ...
    let sourceTask = (0, spawnAsync_1.default)('yes', [], { ignoreStdio: true });
    ...
    let sinkTask = (0, spawnAsync_1.default)('cat');
    (0, assert_1.default)(sourceTask.child.stdout && sinkTask.child.stdin);
...
it(`exports TypeScript types`, async () => {
    let options = {};
    let promise = (0, spawnAsync_1.default)('echo', ['hi'], options);
