...
const ExpoDefinePlugin_1 = __importDefault(require("../plugins/ExpoDefinePlugin"));
...
function withUnimodules(webpackConfig = {}, env = {}, argv = {}) {
    ...
    env.projectRoot = env.projectRoot || webpackConfig.context || (0, paths_1.getPossibleProjectRoot)();
    // Add native react aliases
    webpackConfig = (0, withAlias_1.default)(webpackConfig, (0, env_1.getAliases)(env.projectRoot));
    ...
    if (typeof supportsFontLoading === 'undefined') {
        ...
        if ((0, utils_1.rulesMatchAnyFiles)(webpackConfig, testFontFileNames)) {
            supportsFontLoading = false;
        }
    }
    const { platform = 'web' } = env;
    const config = argv.expoConfig || (0, env_1.getConfig)(environment);
    const mode = (0, env_1.getMode)(env);
    const locations = env.locations || (0, env_1.getPaths)(environment.projectRoot, environment);
