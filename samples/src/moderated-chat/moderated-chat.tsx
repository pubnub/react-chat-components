...
import { ChannelMetadataObject, UUIDMetadataObject, ObjectCustom, GetAllMetadataParameters, ListenerParameters, ObjectsEvent, SetUUIDMetadataEvent, RemoveUUIDMetadataEvent, BaseObjectsEvent, SetUUIDMetadataParameters } from "pubnub";
...
const ThemeContext = createContext<{ value: Themes, set: Dispatch<SetStateAction<Themes>> }>({ value: "light", set: () => {} });
  ...
  return <div className={`app`}>
    <ThemeContext.Provider value={{ value: theme, set: setTheme }}>
      <div className={`modal ${isOpen ? "shown" : ""} ${theme}`}>
        <div className="modal_content-area">
          {modalContent}
        </div>
      </div>
      <ModeratedChatSetup setModalState={setIsOpen} setModalContent={setModalContent}></ModeratedChatSetup>
...
function ModeratedChatSetup ({ setModalState, setModalContent }: ModalConsumerProps) {
  ...
  const params = useMemo(() => ({ uuid: pubnub.getUUID(), include: { channelFields: true, customChannelFields: true } }),[pubnub]) as unknown as GetAllMetadataParameters;
  
  const [rawChannels] = useUserMemberships(params);
  const [updatedChannels, setUpdatedChannels] = useState<{ [id: string]: ChannelMetadataObject<ObjectCustom> }>({});
  ...
  const channels = useMemo(() => rawChannels.map((c) => updatedChannels.hasOwnProperty(c.id) ? merge(c, updatedChannels[c.id]) : c ), [rawChannels, updatedChannels])
  ...
  return <ModeratedChatContent channels={channels} currentUser={currentUser} setModalContent={setModalContent} setModalState={setModalState}/>
}

function FileUpload({ file, setModalState, channel }: {file: File, setModalState: (state: boolean) => void, channel: string}) {
  ...
  return <div className="file-preview">
    <img src={image}/>
    <div className="filename">{file.name} ({size})</div>
    <div className="description">Description <span>(optional)</span></div>
    <TextField emojiPicker={<Picker />} theme={theme} text={text} onChange={setText} onSubmit={send} hideSendButton placeholder=""/>
    <div className="modal_actions">
      <button onClick={() => { setModalState(false); setText(""); }}>Cancel</button>
      <button className="cta" onClick={send}>Upload</button>
    </div>
  </div>
}

function FilePickerButton({id, channel, setModalState, setModalContent }: {id: string, channel: string } & ModalConsumerProps) {

  const showModal: React.ChangeEventHandler<HTMLInputElement> = (e) => {
    setModalState(true);
    setModalContent(<FileUpload file={(e.target?.files || [])[0]} channel={channel} setModalState={setModalState}/>);
  }
...
function FlagUser({ setModalState, sender, currentUser }: {setModalState: (state: boolean) => void, message: Message, sender: UUIDMetadataObject<ObjectCustom>, currentUser: string }) {
  const pubnub = usePubNub();
  const { value: theme } = useContext(ThemeContext);
  const [text, setText] = useState("");
  ...
  const flag = async () => {
    ...
    await pubnub.objects.setUUIDMetadata<UserCustom>({
      uuid: sender.id,
      data: {
        custom: {
          flag: true,
    ...
    } as SetUUIDMetadataParameters<UserCustom>).catch(console.error);
    setModalState(false);
    setText("");
  }
  ...
  return <div className="file-preview">
    ...
    <TextField emojiPicker={<Picker />} theme={theme} text={text} onChange={setText} onSubmit={flag} hideSendButton placeholder=""/>
    <div className="modal_actions">
      <button onClick={() => { setModalState(false); setText(""); }}>Cancel</button>
      <button className="cta" onClick={flag}>Submit</button>
    </div>
  </div>
}
...
function ModeratedChatContent({ currentUser, channels, setModalContent, setModalState }: { channels: ChannelMetadataObject<ObjectCustom>[], currentUser: UUIDMetadataObject<UserCustom> } & ModalConsumerProps) {
  ...
  const [usersUpdated, setUsersUpdated] = useState<{ [id: string]: UUIDMetadataObject<UserCustom> }>({});
  ...
  const users = useMemo(() => usersRaw.map((u) => usersUpdated.hasOwnProperty(u.id) ? merge(u, usersUpdated[u.id]) : u ), [usersRaw, usersUpdated]) as UUIDMetadataObject<UserCustom>[];
  ...
  const blocked = (currentUser.custom?.blockedChannels || "").split(",").includes(currentChannel.id);
  ...
  const presentUsers = useMemo(() => users.filter((u) => presentUUIDs?.includes(u.id) && u.custom?.ban !== true), [presentUUIDs, users]);

  const showFlagUserModal = (message: Message, sender: UUIDMetadataObject<ObjectCustom>) => {
    setModalState(true);
    setModalContent(<FlagUser currentUser={currentUser.id} message={message} sender={sender} setModalState={setModalState}/>);
  }
  ...
  return (
    <div className={`app-simple ${theme}`}>
      ...
      <Chat theme={theme} users={users} currentChannel={currentChannel.id} channels={allChannelIds} onUser={onUser}>
        ...
        <div className="chat">
          ...
          >
            <span><strong>{presentUsers.length || 0}</strong>{membershipCount ? <span>/{membershipCount}</span> : ""}</span>
            <PeopleGroup />
          </div>
          ...
          </MessageList> : <div style={{width: "100%", height: "100%"}}></div>
          }
          <MessageInput typingIndicator emojiPicker={<Picker />} disabled={!canWrite} placeholder={placeholder} additionalActions={[<FilePickerButton id="0" channel={currentChannel.id} setModalContent={setModalContent} setModalState={setModalState}/>]}/>
        </div>
